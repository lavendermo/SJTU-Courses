 ...
 * 
 */
/*||操作：如果第一个是true,只返回第一个的值。如果第一个是false,返回第二个(不管第二个是true还是false)*/
var psd3 = psd3 || {};   
psd3.Graph = function(config) {
    var _this = this;
    this.config = config;
    this.defaults = {
        width: 400,              /**************修改饼状图svg的大小**********************************/
        height: 400,
        value: "value",
        inner: "inner",
        label: function(d) {
        ...
        labelname: "label",
        labelnamefuc: function(d) {
            return d.data.label;
        },
        tooltip: function(d) {
            if (_this.config.value !== undefined) {
                return d[_this.config.labelname]+" "+d[_this.config.value];
            } else {
                return d.value;
            }
...
psd3.Pie.prototype.textTitle = function(d) {
    //console.log(d.data[_this.config.value]+d.data[_this.config.labelname]);
    //console.log(d.data[_this.config.labelname]);
    return d.data[_this.config.labelname]+d.data[_this.config.value];
};
